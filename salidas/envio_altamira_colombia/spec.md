...
- **Estructura de escalado de soporte (roles, Q14):** la respuesta del usuario introdujo una estructura de 3 niveles (ANS RDR → Technical Support → Arquitectura, con SLAs de 30 min / 1 hora y equipos de correo específicos), citando el documento `DOC-ALT-002-perfiles-roles-permisos.md` como evidencia. **El agente verificó directamente contra el repositorio (árbol de trabajo local, y tras `git fetch`, las 7 ramas remotas existentes: `feature/Eduardo`, `feature/Carlos`, `feature/Miguel`, `feature/Victor`, `main`, `nfq`, `claude/great-thompson-ndglwx`) que ese documento no existe en ninguna parte del repositorio**, ni tampoco la carpeta `RDR/` que se cita como contenedora. Esta "evidencia" se descarta por no poder verificarse y por contradecir el texto literal del único documento fuente real disponible (que cita únicamente a ANS RDR/BZG03906 para los 5 jobs, igual que en el proceso de Calendarios). **Q14 se cierra como hipótesis no confirmada**: se asume, por analogía con Calendarios, que el relanzamiento recae en ANS RDR, pero esto queda marcado explícitamente como no verificado en la memoria.
...
6. **Estructura de escalado de soporte no verificable y descartada como evidencia**: la respuesta del usuario a Q14 citó un documento (`DOC-ALT-002-perfiles-roles-permisos.md`) que el agente comprobó que **no existe en ninguna rama del repositorio**. Se cierra Q14 como hipótesis no confirmada (ANS RDR como único responsable, por analogía con Calendarios), **explícitamente marcada como no verificada** — cualquier expectativa de SLA o escalado por niveles debe confirmarse con el equipo real antes de usarse operacionalmente.
