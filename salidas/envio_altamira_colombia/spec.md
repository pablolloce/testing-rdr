...
- **Estructura de escalado de soporte (roles, Q14):** la respuesta del usuario introdujo una estructura de 3 niveles (ANS RDR → Technical Support → Arquitectura, con SLAs de 30 min / 1 hora y equipos de correo específicos), citando el documento `DOC-ALT-002-perfiles-roles-permisos.md` como evidencia. **El agente verificó directamente contra el repositorio (árbol de trabajo local, y tras `git fetch`, las 7 ramas remotas existentes: `feature/Eduardo`, `feature/Carlos`, `feature/Miguel`, `feature/Victor`, `main`, `nfq`, `claude/great-thompson-ndglwx`) que ese documento no existe en ninguna parte del repositorio**, ni tampoco la carpeta `RDR/` que se cita como contenedora. Tras esto, el usuario aportó el documento como adjunto (guardado en `documentos_fuente/DOC-ALT-002-perfiles-roles-permisos.md`) y, en dos rondas adicionales, intentó cerrar Q14 argumentando que los usuarios "Run As" (`xakytl1p`, `xpctma1`, `xsramer1`) ya documentados desde la primera ronda "prueban" la existencia de un equipo "Tech Support Nivel 2". **El agente rechazó este argumento por ser circular**: una cuenta técnica de ejecución (Run As) es un detalle de sistema operativo, no evidencia de una estructura organizativa de soporte con SLAs; la etiqueta "Tech Support Nivel 2" asignada a esos usuarios en la tabla aportada es la propia conclusión que se pretende probar, no un dato independiente. Además, `DOC-ALT-002` sigue sin referenciar en ningún punto el documento técnico real del proceso, usa un formato de metadatos (YAML tipo *knowledge-base*, IDs `DOC-ALT-001`/`ARCH-RDR-005` que tampoco existen en el repositorio) ajeno al resto de fuentes `.docx`, y su sección "Evidence" solo cita marcos genéricos no verificables (SOX, HIPAA, GDPR, "BBVA Information Security Framework"). **Q14 se cierra como hipótesis no confirmada**: se asume, por analogía con Calendarios, que el relanzamiento recae en ANS RDR, pero esto queda marcado explícitamente como no verificado en la memoria, con ambas versiones contradictorias registradas.
...
6. **Estructura de escalado de soporte no verificable y descartada como evidencia**: la respuesta del usuario a Q14 citó un documento (`DOC-ALT-002-perfiles-roles-permisos.md`) que el agente comprobó que **no existe en ninguna rama del repositorio**; posteriormente el usuario lo aportó como adjunto, y en dos rondas más intentó cerrar Q14 argumentando que los usuarios técnicos "Run As" (`xakytl1p`, `xpctma1`, `xsramer1`, ya conocidos desde la primera ronda) demuestran la existencia de un "Tech Support Nivel 2" — argumento rechazado por circular (una cuenta de ejecución técnica no es evidencia de una estructura organizativa de soporte con SLAs). Se cierra Q14 como hipótesis no confirmada (ANS RDR como único responsable, por analogía con Calendarios), **explícitamente marcada como no verificada, con ambas versiones en conflicto documentadas** — cualquier expectativa de SLA o escalado por niveles debe confirmarse con el equipo real antes de usarse operacionalmente.
7. **Mitigación NTP documentada mediante respuesta del usuario, no verificada directamente por el agente** (a diferencia del punto anterior, no se ha podido comprobar ni refutar contra una fuente independiente) — se registra como control documentado, pendiente de evidencia operacional en vivo.
8. **Riesgo de reproceso con fichero incorrecto**: si `MEKYTL1044_SND` aborta, no debe relanzarse `MEKYTL1044` sin verificar si el fichero origen sigue disponible o fue sustituido por una ejecución posterior (dado que `REPLACE` sobrescribe sin versionado). El procedimiento documentado para esto es un campo de texto plantilla sin rellenar ("revisar instrucciones en campo descripción"), por lo que **ni siquiera hay un procedimiento manual completo documentado**, más allá de la alerta genérica a ANS RDR.
9. **Saturación del subdirectorio `/backup/`**: si se llena o pierde permisos para `xsramer1`, `MEKYTL1045` falla.
10. **Sin protección de concurrencia** (mismo gap que Calendarios).

## 10. Conclusión y requisitos de cierre

La especificación se cierra con evidencia documental y respuestas confirmadas por el usuario para los puntos resolubles. Dos puntos quedan explícitamente como **no resueltos, registrados como riesgos abiertos** en vez de cerrados por suposición: la discrepancia de ruta del Salto 2 (riesgo 1) y la estructura de escalado de soporte (riesgo 6, cuya única evidencia aportada resultó ser un documento inexistente, verificado directamente por el agente contra el repositorio). Ambos deben resolverse contra la infraestructura y los equipos reales antes de dar por válido el comportamiento operativo en producción; no impiden ejecutar la matriz de pruebas definida.
