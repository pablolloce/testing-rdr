...
Todos los gaps detectados durante el análisis quedaron resueltos con evidencia (documental, código fuente, o confirmación explícita del usuario en sesión):
...
- **Correspondencia clave↔categoría:** `MEKYTL0967`=Batch Issues, `0968`=Online Issues, `0969`=Batch Issuer, `0970`=Online Issuer, según el orden de la cadena.
...
- **TC-003 (error_funcional):** valida qué ocurre en el envío (`MEKYTL09xx`) cuando el `.tar` de origen no existe por no haberse generado (consecuencia de TC-002) — el job de envío debe terminar en error.
