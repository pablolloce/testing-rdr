...
- Patrón de falacia a vigilar: **usar un dato técnico de ejecución (usuario "Run As" de un job en Control-M) como si fuera prueba de una estructura organizativa de soporte/escalado.** Un "Run As" es la cuenta de sistema operativo bajo la que corre el proceso, no implica pertenencia a un equipo humano con SLA. En Altamira Colombia se intentó cerrar Q14 (dos veces) argumentando que los usuarios `xakytl1p`/`xpctma1`/`xsramer1` (ya documentados como Run As desde la primera ronda) "demuestran" un "Tech Support Nivel 2" — la etiqueta de equipo era la propia conclusión que se pretendía probar, no un dato independiente (razonamiento circular). Rechazado ambas veces; no cerrar un gap organizativo con evidencia puramente técnica de ejecución. _(pablo.llorente, 2026-09-17, Envío a Altamira Colombia)_
...
| Estructura de escalado de soporte (ANS RDR / Technical Support / Arquitectura, SLAs 30min/1h) | Envío a Altamira Colombia | pablo.llorente | Cerrada como hipótesis no confirmada (definitivo, 3 intentos rechazados) | 1) Citaba `DOC-ALT-002-perfiles-roles-permisos.md`, verificado como inexistente en el repositorio. 2) Aportado luego como adjunto, pero sin referenciar el documento técnico real, con formato de metadatos ajeno (YAML knowledge-base) y "Evidence" no verificable. 3) Se intentó cerrar de nuevo argumentando que los usuarios Run As (`xakytl1p`/`xpctma1`/`xsramer1`) prueban un "Tech Support Nivel 2" — razonamiento circular, rechazado. Se asume por defecto, como hipótesis no confirmada, que el relanzamiento recae únicamente en ANS RDR (por analogía con Calendarios), pendiente de confirmación real con el equipo |
